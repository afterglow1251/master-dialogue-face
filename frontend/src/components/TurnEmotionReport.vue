...
function formatVADDelta(current: number, previous: number): string {
  const delta = current - previous;
  const sign = delta >= 0 ? "+" : "";
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="max-h-[80vh] overflow-y-auto p-6 sm:max-w-lg">
      <DialogHeader>
      ...
      <div v-if="turn.mood" class="space-y-3">
        <h4 class="text-sm font-medium">{{ t("emotionReport.moodState") }}</h4>
        <div class="space-y-1.5">
          <div
